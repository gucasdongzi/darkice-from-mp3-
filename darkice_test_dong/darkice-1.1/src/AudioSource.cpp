                                                            ...
                                                            throw ( Exception )
{
    ...
    if(Util::strEq(deviceName,"/home",5)){
	  Reporter::reportEvent(1,"Using MP3 Controlled input:",
	  					deviceName);
	  return new Mp3FileSource(deviceName,
	  	sampleRate,
	  	bitsPerSample,
	  	channel
	  );
   ...
   else if(Util::strEq(deviceName,"circle",6)){
	  Reporter::reportEvent(1,"Using CIRCLE Controlled input:",
	  					deviceName);
	  return new CircleSource(deviceName,
		sampleRate,
	  	bitsPerSample,
	  	channel
	  );
   }
   else if(Util::strEq(deviceName,"epg",3)){
	  Reporter::reportEvent(1,"Using EPG_InfoList Controlled input:",
	  					deviceName);
	  return new EpgSource(deviceName,
	  	sampleRate,
	  	bitsPerSample,
	  	channel
	  );
    }
  else if ( Util::strEq( deviceName, "/dev/tty", 8) ) {
