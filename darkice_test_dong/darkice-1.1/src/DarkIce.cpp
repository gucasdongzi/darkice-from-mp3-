 ...
 *  File identity
 *----------------------------------------------------------------------------*/
static const char fileid[] = "$Id: DarkIce.cpp 488 2010-09-17 20:37:46Z rafael@riseup.net $";
...
DarkIce :: init ( const Config      & config )              throw ( Exception )
{
    ...
    */
    ...
    sampleRate = Util::strToL(strSamples);
    bitsPerSample = Util::strToL(strBits);
    channel = Util::strToL(strChannel);
    device = strDevice;
                                                        ...
                                                        throw ( Exception )
{
	...
	//printf("str=%s\n",str);
        ...
        port        = Util::strToL( str);
        password    = cs->getForSure("password"," missing in section ",stream);
       // mountPoint  = cs->getForSure( "mountPoint"," missing in section ",stream);
...
#else
		{
		printf("%d	%d\n",dsp.get()->getSampleRate(),sampleRate);
