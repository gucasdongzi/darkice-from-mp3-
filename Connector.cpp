 ...
 *  File identity
 *----------------------------------------------------------------------------*/
static const char fileid[] = "$Id: Connector.cpp 474 2010-05-10 01:18:15Z rafael@riseup.net $";
...
Connector :: open ( void )                          throw ( Exception )
{
    ...
    */
    num_not_open=0;
   Reporter::reportEvent(1,"Opening Sinks ",numSinks);
    for ( u = 0; u < numSinks; ++u )
        if ( !sinks[u]->isOpen() ) {
           // printf("sinks u is not open %d\n",u);
            if ( !sinks[u]->open() ) {
               not_open_sink_index[num_not_open++]=u;
               Reporter::reportEvent(1,"Sink Not Open Id is",u);
            }
