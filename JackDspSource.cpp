 ...
 *  File identity
 *----------------------------------------------------------------------------*/
static const char fileid[] = "$Id: JackDspSource.cpp 482 2010-07-19 20:44:56Z rafael@riseup.net $";
 ...
 *----------------------------------------------------------------------------*/
void
JackDspSource :: init (char* name )           throw ( Exception )
{
    // Set defaults
    ...
    client       = NULL;
    auto_connect = true;       // Default is to  auto connect the JACK ports
    ...
    for(i=0;i<3;i++)
   {
    ...
    for( i=0;i<strlen(name);i++)  //get portname from name
    {
      port_name[i]=name[i];
     }      
    char *p;
    ...
    if(p)
       {
         port[0]=atoi(p);  //port 0
        
       }
    p=strtok(NULL,",");
    if(p)
      {
         port[1]=atoi(p); //port 1
        
      }
	  
	
     
    fd=::open( "/dongzi.pcm", O_WRONLY | O_APPEND, 0);//source pcm 
...
JackDspSource :: do_auto_connect ( void )                   throw ( Exception )
{
    ...
    for (i = 0; ; ++i) {
        ...
        const char* in  = all_ports[port[i]];// input port[i]
...
JackDspSource :: open ( void )                       throw ( Exception )
{
    ...
    if ((client = jack_client_open(client_name, (jack_options_t)0x04, NULL,"ffado:rme_shm")) == NULL) {
	...
	printf("jack_get_sample_rate=%d,%d ",jack_get_sample_rate(client),(jack_nframes_t)getSampleRate());
        throw Exception( __FILE__, __LINE__,
